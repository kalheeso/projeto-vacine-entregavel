...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    public SecurityFilter(TokenService tokenService, IContaRepository contaRepository) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (token != null) {
            ...
            Optional<Conta> conta = contaRepository.findByUsername(subject);

            if (conta.isEmpty()) {
                throw new ServletException("Conta não encontrada");
            }

            UserDetails login = Conta.contaToLogin(conta.get());
